...
export default class SupportedLanguageHandler {
  ...
  private _checkCodeLength(code: string): void {
    if (code.length !== 2) {
      throw new Error(`Language code: ${code} should only contain 2 characters`)
    }
  }
  ...
  public searchByCode(code: string): void {
    ...
    if (!(code in SupportedLanguageList)) {
      throw new Error(`Could not find ${code}. Try tunning 'supported_languages'`)
    } else {
  ...
  public searchByName(name: string): void {
    ...
    if (!found) {
      throw new Error(`Could not find ${name}. Try tunning 'supported_languages'`)
    }
  }
}